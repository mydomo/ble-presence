                ...
                if result_string.startswith("[('") and result_string.endswith("'])]"):
                    ...
                    for x in Devices:
                        ...
                        if DEVICE_FOUND == False:
                            ...
                            # This means int's not the first device (generated by the script)
                            if x > 1:
                                # Signal device (update to 0)
                                if str(Devices[x].DeviceID).startswith("S-"):
                                    LASTUPDATE_EXPIRED = time.mktime(datetime.datetime.strptime(Devices[x].LastUpdate, "%Y-%m-%d %H:%M:%S").timetuple())
                                    time_difference_EXPIRED = (round(int(time.time())) - round(int(LASTUPDATE_EXPIRED)))

                                    if (time_difference_EXPIRED > int(Parameters["Mode1"])):
                                        Domoticz.Log( "Device " + Devices[x].DeviceID + " cannot be found and reached the timeout, setting as off/no-signal")
                                        UpdateDevice_by_DEV_ID(Devices[x].DeviceID, 0, str("0"))

                                # If isn't a battery device turn off the main device.
                                elif not (str(Devices[x].DeviceID).startswith("B-")):
                                    LASTUPDATE_EXPIRED = time.mktime(datetime.datetime.strptime(Devices[x].LastUpdate, "%Y-%m-%d %H:%M:%S").timetuple())
                                    time_difference_EXPIRED = (round(int(time.time())) - round(int(LASTUPDATE_EXPIRED)))
