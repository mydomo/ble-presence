...
def socket_input_process(input_string):
    ...
    if input_string == 'beacon_data':
        ...
        if scan_beacon_data == True:
            ...
            return str("Ordered by Timestamp:" + sorted(beacons_detected.items(), key=lambda x: x[1][1], reverse=True)[:300] + "<br> Ordered by RSSI:" + sorted(beacons_detected.items(), key=lambda x: x[0][0], reverse=True)[:300])
